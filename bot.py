...
end_msg = "\n\n> Supporter : `민준` ㅣ [Support Server](https://discord.gg/B48XZe3JQV) ㅣ [Bot Invite](https://discord.com/api/oauth2/authorize?client_id=955403147516448809&permissions=8&scope=bot)"
...
async def on_message(message):
    if (message.content.startswith("!명령어")):
        ...
        await message.reply(embed=discord.Embed(title="Lv Backup service 명령어",description=f"!인증 : 인증을 하실채널을 지정해주세요.\n!복구 복구키 : 자신에게 지급된 복구키로 복구를 시작합니다.\n!정보 :라이센스 정보를 확인합니다.\n!로그웹훅 (웹훅) : 인증로그를 보낼곳을 지정합니다.\n!웹훅보기 : 인증로그가 지정되어있는 웹훅을 보여줍니다.\n!역할 @멘션 : 인증시 부여할 역할을 지정합니다.\n!확인 (확인할서버아이디): 자신의 복구봇의 예상복구인원을 확인가능합니다.{end_msg}", color=0x2f3136))

    if (message.content.startswith("!인증유저")):
        print("1")
        g = message.guild.id
        con,cur = start_db()
        cur.execute("SELECT * FROM users WHERE guild_id == ?;",(g,))
        users = cur.fetchall()
        con.close()
        users = list(set(users))
        await message.channel.send(f"{len(users)}명")
